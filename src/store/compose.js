...
export default {
    ...
    mutations: {
        ...
        // 设置左对齐
        setElementRight ({ componentData, areaData }, align) {
            let common = ''
            if (align == 'left' || align == 'top') {
                common = Math.min.apply(Math, areaData.components.map(item => { return item.style[align] }))
            } else {
                align = align == "right" ? 'left' : 'top'
                common = Math.max.apply(Math, areaData.components.map(item => { return item.style[align] }))
            }
            componentData.forEach(item => {
                areaData.components.forEach(ele => {
                    if (item.id == ele.id) {
                        item.style[align] = common
                    }
                })
            })
        },
