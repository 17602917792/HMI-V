<template>
    ...
    >
        ...
        <ContextMenu @change-picture="hanldeChangePicture" />
        <!-- 标线 -->
        <MarkLine />
        <!-- 选中区域 -->
        <Area
...
import { addGalleryInfos } from '@/api/picture.js'
...
export default {
    ...
    mounted() {
        ...
        eventBus.$on('handleImageDom', () => {
            domtoimage.toPng(this.$refs.editor).then(async (dataUrl) => {
                let fileOfBlob = this.dataURLtoFile(dataUrl)
                const formData = new FormData()
                formData.append('file', fileOfBlob)
                const response = await addGalleryInfos({
                    formData,
                    stationId: this.$route.query.stationId,
                    imageName: Date.now() + '-略缩图',
                })
    ...
    methods: {
        changePicture(e) {
            ...
            reader.onload = (e) => {
                ...
                let curPicture = this.curPicture
                data.forEach(item => {
                    if (curPicture.id === item.id) {
                        item.propValue = base64Str
                    }
                })
                this.$store.commit('setComponentData', data)
                this.dialogVisible = false
                // console.log(this.componentData)
                // console.log('file 转 base64结果：' + base64Str)
            }
        },
        hanldeChangePicture(val) {
        ...
        dataURLtoFile(base64Data) {
            let binary = atob(base64Data.split(',')[1])
            let array = []
            let blob
            ...
            return new File([blob], Date.now() + '.png') // 重命名了
        },
    },
}
</script>
...
<style scoped>
 .editor >>> .avatar-uploader .el-upload {
